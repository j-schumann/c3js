...
class Container
{
    ...
    /**
     * Retrieve the DOM selector used for the chart container.
     *
     * @return type
     */
     ...
     *
     * @param \C3Js\Chart\ChartInterface $chart
     * @return void
     */
    ...
    public function setAxisPadding($axis, $where, $amount)
    {
        if (!in_array($where, array('top', 'bottom', 'left', 'right'))) {
            throw new Exception\InvalidArgumentException('$where has to be one of "top", "left", "right", "bottom"');
        }
        if (!is_numeric($amount)) {
            throw new Exception\InvalidArgumentException('Padding amount has to be float');
        }
    ...
    public function setXAxisType($type)
    {
        if ($type != "timeseries" && $type != "indexed" && $type != "category") {
            throw new Exception\InvalidArgumentException('Invalid X axis type, has to be one of timeseries, indexed or category.');
        }
    ...
    public function toArray($withcharts = true)
    {
        ...
        if ($withcharts) {
            if (empty($this->charts)) {
                throw new Exception\UnderflowException('You cannot convert an empty container to array, add charts or set $withcharts to false when calling toArray.');
            }
